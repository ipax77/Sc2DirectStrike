...
using ParserReplayDto = dsstats.shared.ReplayDto;
using ParserReplayPlayerDto = dsstats.shared.ReplayPlayerDto;
using ParserSpawnDto = dsstats.shared.SpawnDto;
using ParserUnitDto = dsstats.shared.UnitDto;

namespace Sc2DirectStrike.ParserCompareSample.OldStack;

internal static class Program
{
    private const string StackName = "Old";
    private const string ParserName = "dsstats.parser";
    private const string S2ProtocolVersion = "0.9.1.1";

    public static async Task<int> Main(string[] args)
    {
        if (args.Length != 2)
        {
            Console.Error.WriteLine("Usage: Sc2DirectStrike.ParserCompareSample.OldStack <request-json> <result-json>");
            return 2;
        }
        ...
        foreach (DecodedReplay decodedReplay in decodedReplays)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < iterations; i++)
                {
                    ...
                    dto = DsstatsParser.ParseReplay(decodedReplay.Replay);
                    parseStopwatch.Stop();
